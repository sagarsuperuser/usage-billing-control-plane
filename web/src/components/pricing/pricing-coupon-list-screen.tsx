...
function formatDiscountType(coupon: Coupon): string {
  if (coupon.discount_type === "percent_off") return `${coupon.percent_off}% off`;
  return `${(coupon.amount_off_cents / 100).toFixed(2)} ${coupon.currency} off`;
}
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-5 px-4 py-6 md:px-6 lg:px-8">
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-surface shadow-sm">
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              <h1 className="text-sm font-semibold text-text-primary">Coupons{filtered.length > 0 ? ` (${filtered.length})` : ""}</h1>
              <div className="flex items-center gap-2">
                ...
                />
                <Link to="/pricing/coupons/new" className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-blue-600 px-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
                    ...
                    {paginated.map((coupon) => (
                      <tr key={coupon.id} className="cursor-pointer transition hover:bg-surface-secondary" onClick={() => navigate({ to: `/pricing/coupons/${encodeURIComponent(coupon.id)}` })}>
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.1em] ${statusTone(coupon.status)}`}>
                            {coupon.status}
                          </span>
                        </td>
                      </tr>
