    ...
    <div className="text-text-primary">
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-surface shadow-sm">
            ...
            {sessionLoading || jobsQuery.isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 px-5 py-16 text-center">
                ...
                <p className="text-xs text-text-muted">No jobs matched the current filters.</p>
              </div>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border-light">
                  {filteredJobs.map((job) => (
                    ...
                    >
                      <td className="px-5 py-3 font-mono text-xs text-text-muted">{job.id}</td>
                      <td className="px-4 py-3 text-text-muted">{job.customer_id || "-"}</td>
                      <td className="px-4 py-3 text-text-muted">{job.meter_id || "-"}</td>
                      <td className="px-4 py-3">
                        <StatusChip tone={statusTone(job.status)}>{job.status}</StatusChip>
                      </td>
                      <td className="px-4 py-3 text-text-muted">{formatRelativeTimestamp(job.created_at)}</td>
                      <td className="px-4 py-3">
                        <button
                          type="button"
                          ...
                          className="inline-flex h-7 items-center gap-1.5 rounded-md border border-border px-2 text-xs text-text-muted transition hover:bg-surface-secondary disabled:cursor-not-allowed disabled:opacity-40"
                        >
            ...
            {/* Pagination */}
            <div className="flex items-center justify-between border-t border-border px-5 py-2 text-xs text-text-muted">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={!canGoPrev}
                  className="inline-flex h-7 items-center gap-1 rounded-md border border-border px-2 text-text-muted transition hover:bg-surface-secondary disabled:cursor-not-allowed disabled:opacity-50"
                >
      ...
      {createOpen ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-lg rounded-lg border border-border bg-surface shadow-lg">
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              <h2 className="text-sm font-semibold text-text-primary">New replay job</h2>
              <button type="button" onClick={() => setCreateOpen(false)} className="text-text-faint hover:text-text-secondary">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="grid gap-4 px-5 py-4">
              <div className="grid grid-cols-2 gap-3">
                <label className="grid gap-1.5">
                  <span className="text-xs font-medium text-text-muted">Customer ID</span>
                  <input
                    value={createCustomerID}
                    onChange={(e) => setCreateCustomerID(e.target.value)}
                    placeholder="cust_123"
                    data-testid="replay-create-customer-id"
                    className="h-9 rounded-lg border border-border bg-surface px-3 text-sm outline-none ring-slate-400 transition focus:ring-2"
                  />
                </label>
                <label className="grid gap-1.5">
                  <span className="text-xs font-medium text-text-muted">Meter ID</span>
                  <input
                    value={createMeterID}
              ...
              <label className="grid gap-1.5">
                <span className="text-xs font-medium text-text-muted">Idempotency key</span>
                <div className="flex gap-2">
                  <input
                    value={idempotencyKey}
                    onChange={(e) => setIdempotencyKey(e.target.value)}
                    data-testid="replay-create-idempotency-key"
                    className="h-9 flex-1 rounded-lg border border-border bg-surface px-3 text-sm outline-none ring-slate-400 transition focus:ring-2"
                  />
                  <button
                    type="button"
                    onClick={() => setIdempotencyKey(generateReplayIdempotencyKey())}
                    className="h-9 rounded-lg border border-border px-2 text-text-muted transition hover:bg-surface-secondary"
                  >
      ...
      {isTenantSession && drawerOpen && selectedJobID ? (
        <aside
          ...
          className="fixed inset-y-0 right-0 z-30 flex w-full max-w-[480px] flex-col border-l border-border bg-surface shadow-lg"
        >
          <div className="flex items-center justify-between border-b border-border px-5 py-3">
            <h2 className="text-sm font-semibold text-text-primary">{selectedJob?.id || selectedJobID}</h2>
            <button type="button" aria-label="Close" onClick={() => setDrawerOpen(false)} className="text-text-faint hover:text-text-secondary">
              <X className="h-4 w-4" />
            </button>
          </div>
            ...
            {diagnosticsQuery.data ? (
              <div className="space-y-5">
                <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  <div className="flex justify-between"><dt className="text-text-muted">Customer</dt><dd className="text-text-secondary">{diagnosticsQuery.data.job.customer_id || "-"}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Meter</dt><dd className="text-text-secondary">{diagnosticsQuery.data.job.meter_id || "-"}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Step</dt><dd className="text-text-secondary">{diagnosticsQuery.data.job.workflow_telemetry?.current_step || diagnosticsQuery.data.job.status}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Progress</dt><dd className="text-text-secondary">{diagnosticsQuery.data.job.workflow_telemetry?.progress_percent ?? 0}%</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Records</dt><dd className="text-text-secondary">{diagnosticsQuery.data.job.workflow_telemetry?.processed_records ?? diagnosticsQuery.data.job.processed_records}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Created</dt><dd className="text-text-secondary">{formatExactTimestamp(diagnosticsQuery.data.job.created_at)}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Last attempt</dt><dd className="text-text-secondary">{diagnosticsQuery.data.job.last_attempt_at ? formatExactTimestamp(diagnosticsQuery.data.job.last_attempt_at) : "-"}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Usage events</dt><dd className="text-text-secondary">{diagnosticsQuery.data.usage_events_count}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Usage qty</dt><dd className="text-text-secondary">{diagnosticsQuery.data.usage_quantity}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Billed entries</dt><dd className="text-text-secondary">{diagnosticsQuery.data.billed_entries_count}</dd></div>
                  <div className="flex justify-between"><dt className="text-text-muted">Billed cents</dt><dd className="text-text-secondary">{diagnosticsQuery.data.billed_amount_cents}</dd></div>
                </dl>
                ...
                {diagnosticsQuery.data.job.artifact_links ? (
                  <div>
                    <h3 className="text-xs font-semibold uppercase tracking-[0.1em] text-text-faint">Artifacts</h3>
...
function ArtifactLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex h-7 items-center gap-1.5 rounded-md border border-border px-2 text-xs text-text-muted transition hover:bg-surface-secondary"
    >
