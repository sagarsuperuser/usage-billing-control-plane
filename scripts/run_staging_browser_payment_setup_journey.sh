...
PLAYWRIGHT_LIVE_PAYMENT_SETUP_CUSTOMER_EXTERNAL_ID="$(jq -r '.customer.external_id // empty' "$fixture_json_file")"
PLAYWRIGHT_LIVE_PAYMENT_SETUP_LAGO_ORG_ID="$(jq -r '.bootstrap_organization_id // empty' "$fixture_json_file")"
PLAYWRIGHT_LIVE_PAYMENT_SETUP_STRIPE_PROVIDER_CODE="$(jq -r '.stripe_provider_code // empty' "$fixture_json_file")"
if [[ -z "$PLAYWRIGHT_LIVE_PAYMENT_SETUP_INVOICE_ID" || -z "$PLAYWRIGHT_LIVE_PAYMENT_SETUP_CUSTOMER_EXTERNAL_ID" || -z "$PLAYWRIGHT_LIVE_PAYMENT_SETUP_LAGO_ORG_ID" || -z "$PLAYWRIGHT_LIVE_PAYMENT_SETUP_STRIPE_PROVIDER_CODE" ]]; then
  echo "failed to derive browser payment-setup fixture env from fixture output" >&2
  exit 1
fi
...
(
  ...
  PLAYWRIGHT_LIVE_PAYMENT_SETUP_CUSTOMER_EXTERNAL_ID="$PLAYWRIGHT_LIVE_PAYMENT_SETUP_CUSTOMER_EXTERNAL_ID" \
  PLAYWRIGHT_LIVE_PAYMENT_SETUP_LAGO_ORG_ID="$PLAYWRIGHT_LIVE_PAYMENT_SETUP_LAGO_ORG_ID" \
  PLAYWRIGHT_LIVE_PAYMENT_SETUP_STRIPE_PROVIDER_CODE="$PLAYWRIGHT_LIVE_PAYMENT_SETUP_STRIPE_PROVIDER_CODE" \
  LAGO_API_KEY="$LAGO_API_KEY" \
