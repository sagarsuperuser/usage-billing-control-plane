        ...
        {isTenantSession ? (
          detailQuery.isLoading ? (
            ...
            <section className="rounded-lg border border-border bg-surface shadow-sm p-5">
              ...
              <p className="mt-1 text-sm text-text-muted">The requested subscription could not be loaded from the workspace APIs.</p>
            </section>
          ) : (
            ...
            <div className="overflow-hidden rounded-lg border border-border bg-surface shadow-sm divide-y divide-border">
              ...
              <div className="px-5 py-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <h1 className="text-base font-semibold text-text-primary truncate">{subscription.display_name}</h1>
                    <span className="font-mono text-xs text-text-faint">{subscription.code}</span>
              ...
              <div className="px-5 py-4">
                <dl className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-3">
                  <div>
                    <dt className="text-xs text-text-faint">Plan</dt>
                    <dd data-testid="subscription-plan-name" className="mt-0.5 text-sm text-text-secondary">{subscription.plan_name}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Plan code</dt>
                    <dd data-testid="subscription-plan-code" className="mt-0.5 text-sm font-mono text-text-secondary">{subscription.plan_code}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Customer</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{subscription.customer_display_name}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Customer ID</dt>
                    <dd className="mt-0.5 text-sm font-mono text-text-secondary">{subscription.customer_external_id}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Base price</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{(subscription.base_amount_cents / 100).toFixed(2)} {subscription.currency}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Billing interval</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{subscription.billing_interval}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Payment setup</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{formatSubscriptionPaymentSetupStatus(subscription.payment_setup_status)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Payment method</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{subscription.default_payment_method_verified ? "Verified" : "Waiting on payer"}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Activated</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{formatExactTimestamp(subscription.activated_at)}</dd>
                  </div>
                </dl>
              </div>
              ...
              {/* ---- Payment setup state ---- */}
              <div className="px-5 py-4">
                <p className="text-xs font-medium text-text-faint mb-3">Payment setup state</p>
                <dl className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-3">
                  <div>
                    <dt className="text-xs text-text-faint">Last verified</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{formatExactTimestamp(subscription.payment_setup.last_verified_at)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Last result</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{subscription.payment_setup.last_verification_result || "-"}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Last error</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{subscription.payment_setup.last_verification_error || "-"}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-text-faint">Requested at</dt>
                    <dd className="mt-0.5 text-sm text-text-secondary">{formatExactTimestamp(subscription.payment_setup_requested_at)}</dd>
                  </div>
                </dl>
                {latestSetupCheckoutURL ? (
                  <div className="mt-3">
                    <a href={latestSetupCheckoutURL} target="_blank" rel="noreferrer" className="inline-flex h-8 items-center rounded-md border border-border bg-surface px-3 text-xs font-medium text-text-secondary transition hover:bg-surface-secondary">
              ...
              {/* ---- Change plan ---- */}
              <div className="px-5 py-4">
                <p className="text-xs font-medium text-text-faint mb-3">Change plan</p>
                <div className="flex flex-wrap items-end gap-3">
                  <label className="grid gap-1 text-sm min-w-[200px] flex-1">
                    <span className="text-xs text-text-faint">Target plan</span>
                    <select
                      ...
                      className="h-9 rounded-md border border-border bg-surface px-3 text-sm text-text-primary outline-none ring-slate-400 transition focus:ring-2 disabled:cursor-not-allowed disabled:bg-surface-secondary"
                    >
