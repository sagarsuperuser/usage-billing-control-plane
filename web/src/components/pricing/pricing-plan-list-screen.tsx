          ...
          <div className="overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-stone-200 px-5 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Link href="/pricing/plans/new" className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-900 bg-slate-900 px-3 text-sm font-medium text-white transition hover:bg-slate-800">
                    ...
                    {paginated.map((plan) => (
                      <tr key={plan.id} className="transition hover:bg-stone-50">
                        <td className="px-5 py-3">
                          <Link href={`/pricing/plans/${encodeURIComponent(plan.id)}`} className="block font-medium text-slate-900">
                            {plan.name}
                          </Link>
                        </td>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-3 px-5 py-16 text-center">
      ...
      <p className="text-xs text-slate-500">Create the first plan after you have at least one metric.</p>
      <Link href="/pricing/plans/new" className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-900 bg-slate-900 px-4 text-sm font-medium text-white transition hover:bg-slate-800">
        <Plus className="h-3.5 w-3.5" />
        New plan
      </Link>
    </div>
