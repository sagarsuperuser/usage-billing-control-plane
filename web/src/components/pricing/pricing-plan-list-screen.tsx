    ...
    <PageContainer>
        ...
        <Card>
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              <h1 className="text-sm font-semibold text-text-primary">Plans{filtered.length > 0 ? ` (${filtered.length})` : ""}</h1>
              <div className="flex items-center gap-2">
                ...
                />
                <Link to="/pricing/plans/new" className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-blue-600 px-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
                  ...
                  <tbody className="divide-y divide-border-light">
                    {paginated.map((plan) => (
                      <tr key={plan.id} className="cursor-pointer transition hover:bg-surface-secondary" onClick={() => navigate({ to: `/pricing/plans/${encodeURIComponent(plan.id)}` })}>
                        ...
                        <td className="px-4 py-3 text-text-muted">{plan.billing_interval}</td>
                        <td className="px-4 py-3 text-text-muted">{(plan.base_amount_cents / 100).toFixed(2)} {plan.currency.toUpperCase()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
