...
func main() {
	...
	envCfg := appconfig.LoadBootstrapPlatformAdminConfigFromEnv()
	...
	flag.StringVar(&name, "name", envCfg.KeyName, "display name for the new platform admin key")
	...
	flag.BoolVar(&allowExistingActiveKeys, "allow-existing-active-keys", envCfg.AllowExistingActiveKeys, "allow bootstrap even when active platform keys already exist")
	flag.Parse()
	...
	keyService := service.NewPlatformAPIKeyService(repo)

	activeKeys, err := keyService.CountActivePlatformAPIKeys()
	if err != nil {
		fatal(logger, "inspect existing active platform keys", "error", err)
	}
	if activeKeys > 0 && !allowExistingActiveKeys {
		fatal(logger, "active platform keys already exist; refusing bootstrap", "active_keys", activeKeys)
	}

	created, err := keyService.CreatePlatformAPIKey(service.CreatePlatformAPIKeyRequest{
		Name:      name,
		Role:      "platform_admin",
		ExpiresAt: expiresAt,
	})
	if err != nil {
		fatal(logger, "create platform admin key", "error", err)
	}
