...
PLATFORM_KEY_NAME="${PLATFORM_KEY_NAME:-staging-payment-setup-journey-platform-admin}"
WRITER_KEY_NAME="${WRITER_KEY_NAME:-staging-payment-setup-journey-writer}"
READER_KEY_NAME="${READER_KEY_NAME:-staging-payment-setup-journey-reader}"
...
(
  ...
  TARGET_TENANT_ID="$TARGET_TENANT_ID" \
  bash ./scripts/verify_staging_payment_setup_journey.sh
)
