    ...
    <div className="min-h-screen bg-[#f5f7fb] text-slate-900">
      <main className="mx-auto flex max-w-[1360px] flex-col gap-5 px-4 py-6 md:px-6 lg:px-8">
        ...
        {isAuthenticated && scope !== "tenant" ? (
          <ScopeNotice title="Tenant session required" body="Metrics are tenant-scoped. Sign in with a tenant account to inspect them." actionHref="/billing-connections" actionLabel="Open platform home" />
        ) : null}
        ...
        {query.isLoading ? (
          ...
          <>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Operator note</p>
              <p className="mt-3 text-sm leading-relaxed text-slate-600">
                Alpha created the underlying pricing draft automatically. Plans can now reuse this metric without exposing lower-level billing-engine objects in the primary workflow.
              </p>
            </section>
          </>
        )}
