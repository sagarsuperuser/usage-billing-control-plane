...
function MetaRow({ label, value, mono = false }: { label: string; value?: string; mono?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-3 rounded-xl border border-border bg-surface-secondary px-4 py-3">
      <span className="text-xs font-semibold uppercase tracking-[0.16em] text-text-muted">{label}</span>
      <span className={`text-right text-sm text-text-primary ${mono ? "font-mono text-xs" : ""}`}>{value || "-"}</span>
    </div>
  );
}
...
export function DunningSummaryPanel({
  ...
  return (
    <section className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-text-muted">Dunning</p>
      ...
      {runHref ? (
        <Link
          to={runHref}
          className="mt-4 inline-flex h-10 w-full max-w-full items-center justify-center rounded-lg border border-border bg-surface-secondary px-4 text-sm font-medium text-text-secondary transition hover:bg-surface-tertiary"
        >
          Open dunning run
        </Link>
      ) : null}
      {summary.next_action_type === "collect_payment_reminder" && onSendReminder ? (
        <button
          ...
          className="mt-4 inline-flex h-10 w-full max-w-full items-center justify-center gap-2 rounded-lg border border-blue-600 bg-blue-600 px-4 text-sm font-medium text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
