...
export function AppBreadcrumbs({ items }: { items: BreadcrumbItem[] }) {
  return (
    <nav
      ...
      className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.14em] text-slate-500"
    >
        ...
        return (
          <div key={`${item.label}-${index}`} className="flex items-center gap-2">
            {item.href && !last ? (
              ...
              <span className={last ? "text-slate-900" : undefined}>{item.label}</span>
            )}
